...
@Seq2VecEncoder.register("bert_encoder")
class BERTSeq2VecEncoder(Seq2VecEncoder):
    """
    # Parameters
    ...
    def generate_square_subsequent_mask(
        self, batch_size: int, src_len: int, device: str = "cpu"
    ) -> torch.Tensor:
        return torch.triu(
    ...
    def forward(self, inputs: torch.Tensor, mask: torch.BoolTensor):
        ...
        output = self._transformer(output, src_key_padding_mask=src_key_padding_mask)
        output = output.permute(1, 0, 2)
        masked_output = output * mask.unsqueeze(-1)
        if self._pooling_method == "mean":
            length_per_batch = mask.sum(dim=1).unsqueeze(-1)
            output = masked_output.sum(dim=1) / length_per_batch
        elif self._pooling_method == "max":
            output = masked_output.max(dim=1)[0]
