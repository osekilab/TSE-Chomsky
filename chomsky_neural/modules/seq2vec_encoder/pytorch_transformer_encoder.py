...
@Seq2VecEncoder.register("pytorch_transformer")
class PytorchTransformerSeq2VecEncoder(Seq2VecEncoder):
    """
    # Parameters
    ...
    def forward(self, inputs: torch.Tensor, mask: torch.BoolTensor):
        output = inputs
        last_indices = mask.sum(dim=1).cpu() - 1
        ...
        output = output[
            torch.arange(batch_size, device=output.device), last_indices, :
